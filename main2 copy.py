...
for i in range(1, last_frame):
    ...
    # Update the continuous operation with the new tracked points
    # continuous.S['P'] = next_pts.T
    assert continuous.S['P'].shape[1] > 100, f"Assertion failed: The number of keypoints is {continuous.S['P'].shape[1]} <= 100, at frame {i}"
    ...
    # axes[0].scatter(old_pts[:, 0], old_pts[:, 1], c='r', s=5, marker='x')
    # axes[0].set_title("Frame 1 Keypoints")
    # axes[0].axis('off')

    # Second frame with displacements
    axes[1].imshow(img2, cmap='gray')
    axes[1].scatter(next_pts[:, 0], next_pts[:, 1], c='g', s=5, marker='x')  # Keypoints in green
    ...
    if cv2.waitKey(500) & 0xFF == ord('q'):  # Wait 0.5 seconds
        break
